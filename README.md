...
一个跑在本地的小工具，用 Electron 套了层壳，主要给自己和团队看抖音账号的内容表现。数据抓回来后做看板、列表和导出，不依赖任何第三方服务。所有数据（包括账号登录态）都只留在本地，不上传任何服务器。
...
本项目涉及抖音数据的本地抓取，使用前请务必了解以下风险与应对方式：

- ⚠️ **账号封禁风险**：本项目通过模拟登录态获取数据，可能违反抖音用户协议。使用过程中账号存在被限流、风控甚至封禁的风险。**请务必使用小号或非重要账号进行测试，切勿使用主账号。** 由此导致的账号损失由使用者自行承担。

- 📵 **收到侵权投诉 / DMCA 下架通知**：若开发者或用户收到平台、权利方或相关主体的侵权投诉、DMCA 删除通知或其他法律函件，应**立即停止相关数据的抓取与分发**，删除本地已存储的相关内容，并配合权利方与平台的处理要求。本项目不提供任何对抗风控、规避下架或绕过平台限制的技术支持。

- 🔐 **隐私与数据合规**：严禁使用本项目抓取、存储、传播他人个人隐私信息（如手机号、身份证号、住址、通讯录等）。因违规使用导致的任何法律责任由使用者自行承担。
...
本仓库的所有内容仅供学习和参考之用，禁止用于商业用途。任何人或组织不得将本仓库的内容用于非法用途或侵犯他人合法权益。本仓库所涉及的爬虫技术仅用于学习和研究，不得用于对其他平台进行大规模爬虫或其他非法行为。本项目通过模拟登录态获取数据，可能违反相关平台用户协议，使用过程中账号存在被限流、风控或封禁的风险，使用者应自行承担由此产生的一切后果，开发者不建议使用主账号运行本项目。
